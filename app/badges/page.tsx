...
import { hasAnyMilestone } from '@/lib/db/sessions';
import { listSkills } from '@/lib/db/skills';
import { computeBadges, type BadgeStatus } from '@/lib/services/badges';

export default async function BadgesPage() {
  const { userId, profile, onboarded } = await getSessionContext();
  if (!onboarded) redirect('/onboarding');

  const [skills, milestone] = await Promise.all([
    listSkills(userId),
    hasAnyMilestone(userId),
  ]);
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-1 flex-col gap-1">
        ...
        <p
          className={`text-sm ${earned ? 'text-violet-900/80' : 'text-violet-900/60'}`}
        >
          {badge.description}
        </p>
        {!earned && progress ? (
