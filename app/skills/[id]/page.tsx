  ...
  if (!skill) {
    return (
      <section className="flex flex-col items-center gap-4 py-12 text-center">
        <h1 className="text-xl font-semibold tracking-tight text-pink-900">
          Skill not found
        </h1>
        ...
        <Link
          href="/skills"
          className="rounded-full bg-pink-600 px-5 py-2 text-sm font-medium text-white shadow-sm transition hover:bg-pink-700"
        >
          Back to skills
        </Link>
      </section>
  ...
  return (
    <section className="flex flex-col gap-6">
      <div>
        <Link
          href="/skills"
          className="inline-flex items-center text-sm text-pink-700 hover:text-pink-900"
        >
      ...
      <header className="flex flex-col gap-3">
        <div className="flex flex-wrap items-center gap-2">
          <span
            className="inline-flex items-center gap-1.5 rounded-full bg-pink-100 px-2.5 py-0.5 text-xs font-medium text-pink-700"
          >
          ...
          {skill.isCurrentlyWorkingOn ? (
            <span
              className="inline-flex items-center gap-1 rounded-full bg-pink-100 px-2.5 py-0.5 text-xs font-medium text-pink-700"
        ...
        <div className="flex items-center gap-2">
          <h1 className="text-xl font-semibold tracking-tight text-pink-900">{skill.name}</h1>
          <PronounceButton text={skill.name} />
        </div>
        <div className="flex items-center gap-2 text-sm text-pink-900/70">
      ...
      {skill.description ? (
        <div className="rounded-2xl border border-pink-200 bg-white p-5 shadow-sm">
          <h2 className="text-sm font-medium text-pink-900/70">About</h2>
          <p className="mt-2 text-pink-950">{skill.description}</p>
        </div>
      ) : null}

      {skill.techniqueTips.length > 0 ? (
        <div className="rounded-2xl border border-pink-200 bg-white p-5 shadow-sm">
          <h2 className="text-sm font-medium text-pink-900/70">Technique tips</h2>
          <ul className="mt-2 list-disc space-y-1 pl-5 text-pink-950">
      ...
      <div className="rounded-2xl border border-pink-200 bg-white p-5 shadow-sm">
        <h2 className="text-sm font-medium text-pink-900/70">Last tried</h2>
        <p className="mt-1 text-pink-950">{humanizeLastAttempted(skill.lastAttemptedAt)}</p>
      </div>
    </section>
  );
}
