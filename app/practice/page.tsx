...
export default async function PracticeIndexPage() {
  ...
  return (
    <section className="flex flex-col gap-6">
      <header>
        <p className="text-sm text-pink-900/60">Ready to dance?</p>
      ...
      {plans.length === 0 ? (
        ...
        <div className="flex flex-col gap-4">
          <h2 className="text-lg font-medium text-pink-900/80">Plans</h2>
          {plans.map((plan) => {
                ...
                <div className={`${CARD_CLASS} flex flex-col gap-3`}>
                  <div className="flex flex-wrap items-start justify-between gap-3">
                    <div>
                      ...
                      {plan.description ? (
                        <p className="mt-1 text-sm text-pink-900/70">{plan.description}</p>
                      ) : null}
                    </div>
                    {plan.isBuiltIn ? (
                      <span className="rounded-full bg-pink-100 px-3 py-1 text-xs font-medium text-pink-700">
                  ...
                  <div className="flex flex-wrap gap-2 text-xs text-pink-900/70">
                    ...
                    {categories.slice(0, 3).map((label) => (
                      <span
                        key={label}
                        className="rounded-full bg-pink-50 px-3 py-1"
                      >
                        {label}
                      </span>
                    ))}
      ...
      <p className="text-center text-sm text-pink-900/60">
